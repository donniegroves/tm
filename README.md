...
**note: keep in mind that columns in Views with Postgres are always nullable**

For more info, visit [Supabase TypeScript Support](https://supabase.com/docs/reference/javascript/typescript-support)

Seed file can be generated by populating the data in the database and then (while docker is running) use `npx supabase db dump -f supabase/seed.sql --data-only`
